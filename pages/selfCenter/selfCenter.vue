<template>
  <view>
    <usershow
      ...
      :userId="id"
      :guanzhu="guanzhu"
      :looktotals="looktotals"
      :lookerCount="lookerCount"
      :likeCount="likeCount"
      :newFans="newFans"
      v-on:clidtags="clidtags"
      :num="num"
      v-on:clickMe="clickMe"
      :isEditInfo="true"
    ></usershow>
    <view class="moreInfo">
      <view class="moreInfoRow2">
        <view class="user_column_item" @tap="gomessage">
              <button class="content cu-btn" open-type="msgcount">
                  <text
              class="lg text-gray cuIcon-mail"
              style="color: red"
            ></text>
                  <text class="thin"
              >消息提醒
        ...
        <view class="user_column_item" @tap="gomoney">
          <button class="content cu-btn">
                  <text class="lg text-gray cuIcon-moneybag"></text>       <text
              class="thin"
              >钱包</text
            >
            <text class="lg text-orange cuIcon-pay margin-left-lg"
              >{{ rmb }}元</text
            >
            <view
              class="cu-tag tag-text1 bg-red"
              v-if="wg_num > 0 && wg_num < 100"
              >{{ wg_num }}</view
            >
            <view class="cu-tag tag-text1 bg-red" v-if="wg_num >= 100"
              >99+</view
            >
                
          </button>
        ...
        <view class="user_column_item">
              <button class="content cu-btn" open-type="feedback">
                  <text class="lg text-gray cuIcon-question"></text>       <text
              class="thin"
              >问题反馈</text
            >
                
        ...
        <view v-if="showHzGroup" class="user_column_item" @tap="gogroup">
              <button class="content cu-btn">
                  <text class="lg text-gray cuIcon-group"></text>       <text
              class="thin"
              >互助小圈</text
            >
              
  ...
  methods: {
    ...
    getUnreadMsgCount(userId) {
      this.xd_request_get(
		...
		console.log(count);
		(this.$data.unreadMsg = count), (this.$data.newFans = newFans);
      });
    },
    ...mapMutations(['logOut']),
    async getBalance() {
      ...
      this.xd_request_post(
        this.xdServerUrls.xd_inquireBalance,
        {
